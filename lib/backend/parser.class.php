 ...
 */
 ...
 function tpluserAvatar($params, &$smarty) {
	if($params['k'] =="") $params['k']= UID;
	$key = $params['k'];
	$path = (ROOT!='../'? AVATARS:"./tmp/avatars/");
	$ava = $path.md5($key);
	if(file_exists($ava) && $ava != $path) {
		return $ava;
	} else {
...
function tplcomments($params, &$smarty) {
	...
	if($params['layout'] =="") $params['layout']= tplCfg('commentsDefault');
