...
class session {
    ...
    function changeTemplate($tpl = false) {
        $db = _new("db");
        if ($tpl) {
            $db->update("user", array("template"), array($tpl), "WHERE `ID` = '" . UID . "'");
        } else {
            $db->update("user", array("template"), array(defaultTpl), "WHERE `ID` = '" . UID . "'");
        }
